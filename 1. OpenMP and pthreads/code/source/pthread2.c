//Implementation: 1 task = 1 string from set A compared with 1 string from set B
...
void *ham_fun(void *param)
{
	...
	/*
	HamTable is being process with a stride equal with the number
	of threads and a different initial value for each thread
	*/
	if (n > 1) {
		while (i < m) {
		
			int j = array_cells_counter % n;
			
			for(int c = 0; c < l; c++)
				if (another_object->A[c][i] != another_object->B[c][j])
					another_object->Ham[i][j]++; 
			
			array_cells_counter = array_cells_counter + cores;	
			
			if ((array_cells_counter % n) <= j){
				i++;
			}
		}	
	}else{
	// stride across i
		while (array_cells_counter < m) {
		
			i = array_cells_counter % m;			
			for(int c = 0; c < l; c++)
				if (another_object->A[c][i] != another_object->B[c][0])
					another_object->Ham[i][0]++;
