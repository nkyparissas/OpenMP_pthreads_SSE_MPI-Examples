...
int main(int argc, char ** argv)
{
	...
	printf("The world has %d processes, I am process No. %d\n", world_size, world_rank);
	...
	for(int j=0; j<iters; j++){
		...
		// Split the work load in equal parts, every process undertakes one part depending on its rank
		for(int i = (world_rank*((N/4)/(float)world_size)); i < floor((world_rank+1)*((N/4)/(float)world_size)); i++){
		...
		// Send the max value to process 0
		if (world_rank != 0) MPI_Send(&maxF, 1, MPI_FLOAT, 0, 0, MPI_COMM_WORLD);
		
		
		if (world_rank == 0) {
			
			// Receive all the max values from the rest of the processes
			for (int i = 1; i<world_size; i++){
			
				float tempMax; 
				MPI_Recv(&tempMax, 1, MPI_FLOAT, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
				
				if (tempMax > maxF) maxF = tempMax;		
			}
	...
	if (world_rank == 0) printf("Time %f Max %f\n", timeTotal/iters, maxF);
